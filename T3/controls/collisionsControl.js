...
function checkCollisionsTankWall(tank, bbTank, bbWalls) {
  for (let i = 0; i < bbWalls.length; i++) {
    ...
    if (bbWall.intersectsBox(bbTank)) {
      const normals = bbWall.normals; // Array de normais da parede com a qual houve colisão
      if (normals && normals.length > 0) {
        for (let j = 0; j < normals.length; j++) {
          ...
          // Opcional: se quiser interromper após a primeira colisão
          // break;
        }
      } else {
        // Se a parede não tiver normais, restauramos a posição anterior
        console.warn("Normais da parede não definidas em checkCollisionsTankWall");
  ...
  for (let i = 0; i < movingWalls.length; i++) {
    ...
    if (bbMovingWall.intersectsBox(bbTank)) {
      ...
      if (movingWall.normal) {
        ...
        console.warn("Normal da parede móvel não definida em checkCollisionsTankMovingWall");
...
function checkCollisions(index, tank1, bbTank1, tank2, bbTank2, tank3, bbTank3, tank4, bbTank4, bbWalls, movingWalls, bbMovingWalls) {
  if (index === 0) {
    ...
    if (tank2.visible && tank3.visible) checkCollisionsTankTank(tank2, bbTank2, tank3, bbTank3);

    // Verificar colisões entre tanques e paredes
    checkCollisionsTankWall(tank1, bbTank1, bbWalls);
    if (tank2.visible) checkCollisionsTankWall(tank2, bbTank2, bbWalls);
    if (tank3.visible) checkCollisionsTankWall(tank3, bbTank3, bbWalls);
  } else if (index === 2) {
    // Verificar colisões entre tanques
    if (tank2.visible) checkCollisionsTankTank(tank1, bbTank1, tank2, bbTank2);
    if (tank3.visible) checkCollisionsTankTank(tank1, bbTank1, tank3, bbTank3);
    if (tank4.visible) checkCollisionsTankTank(tank1, bbTank1, tank4, bbTank4);

    if (tank2.visible && tank3.visible) checkCollisionsTankTank(tank2, bbTank2, tank3, bbTank3);
    if (tank2.visible && tank4.visible) checkCollisionsTankTank(tank2, bbTank2, tank4, bbTank4);
    if (tank3.visible && tank4.visible) checkCollisionsTankTank(tank3, bbTank3, tank4, bbTank4);
